...
def query_sort_key(query_id: Any) -> tuple[int, Any]:
    """Sort numeric-looking query IDs numerically and others lexicographically."""
    query_id_str = str(query_id)
    if query_id_str.isdigit():
        return 0, int(query_id_str)
    return 1, query_id_str


def build_edge_list_e_by_2(edge_index: Any, query_id: Any) -> np.ndarray:
    ...
    if edge_array.ndim != 2:
        raise ValueError(
            f"Unexpected edge_index rank for query_id={query_id}: "
            f"{edge_array.shape}"
        )
...
def convert_pt_to_reqo_npy(
    ...
    for data in data_list:
        query_id = get_data_attr(data, "query_id")
        if query_id is None:
            metadata = get_data_attr(data, "metadata", {}) or {}
            query_id = metadata.get("query_id")
        if query_id is None:
            raise ValueError("Found a data item without query_id.")
        grouped.setdefault(query_id, []).append(data)
    ...
    for query_id in sorted(grouped, key=query_sort_key):
        ...
        for plan_idx, data in enumerate(group):
            ...
            edge_list_e_by_2 = build_edge_list_e_by_2(data.edge_index, query_id)
            dataset_rows.append([x, edge_list_e_by_2, float(runtime)])

            if "sql_id" in metadata:
                plan_id = metadata["sql_id"]
            else:
                plan_id = get_data_attr(data, "plan_id", plan_idx)
