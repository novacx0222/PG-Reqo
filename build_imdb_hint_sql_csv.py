...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build full hint-augmented IMDb SQL CSVs from exported hints."
    )
    parser.add_argument(
        "--results-path",
        type=Path,
    ...
    parser.add_argument(
        ...
        type=Path,
        required=True,
        help="Directory where one full CSV per parameter group will be written.",
    )
    parser.add_argument(
        "--parameter-groups",
        nargs="+",
...
def validate_args(args: argparse.Namespace) -> None:
    """Validate arguments before scanning result directories."""
...
def build_group_csv(
    ...
    """Write one parameter group's full hint-augmented SQL CSV."""
...
def main() -> None:
    ...
    for parameter_group_dir in parameter_group_dirs:
        output_csv, query_group_count, row_count = build_group_csv(
            parameter_group_dir=parameter_group_dir,
            results_path=results_path,
            output_dir=output_dir,
        ...
        print(
            ...
            f"query_groups={query_group_count}, rows={row_count}, "
            f"csv={output_csv}"
        )

    print(f"Total query groups: {total_query_groups}")
